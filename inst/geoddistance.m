...
% Copyright (c) Charles Karney (2012-2025) <karney@alum.mit.edu>.
%
...
% to allow a vectorized solution.
  ...
  if any(m)
    ...
    sig12(m) = ...
        atan2(max(0, csig1(m) .* ssig2(m) - ssig1(m) .* csig2(m)) + 0, ...
...
%LAMBDA12  Solve the hybrid problem
  ...
  sig12 = atan2(max(0, csig1 .* ssig2 - ssig1 .* csig2) + 0, ...
                       csig1 .* csig2 + ssig1 .* ssig2);

  somg12 = max(0, comg1 .* somg2 - somg1 .* comg2) + 0;
